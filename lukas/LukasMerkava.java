...
public class LukasMerkava extends AdvancedRobot {
    ...
    public void onScannedRobot(ScannedRobotEvent e) {
        ...
        this.setTurnRightRadians(absbearing - Math.PI / 2);
        pattern.insert(0, (char)Math.round(Math.sin(e.getHeadingRadians() - (absbearing += this.getHeadingRadians())) * e.getVelocity()));
        int index;
        int searchlength = 30;
        while ((index = pattern.toString().indexOf(pattern.substring(0, searchlength--), 1)) < 0 && searchlength > 0) {}
        double dist = e.getDistance();
        double power = Math.min(3, Math.min(this.getEnergy(), e.getEnergy()) / (double)4);
